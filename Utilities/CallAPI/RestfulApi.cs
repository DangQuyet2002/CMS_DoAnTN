...
namespace Utilities
{
    ...
    {
        private static readonly string _apiUrl = ConfigurationManager.AppSettings["is:ApiUrl"];
        /// <summary>
        ...
        public static void AddProperty(object expando, string propertyName, object propertyValue)
        {
            if (expando != null)
            {
        ...
        private static async Task<T> RestSharp(string apicontroller, object dataObject, Method method, string apiUrl = null)
        {
            if (dataObject != null)
            {
                ...
                foreach (var property in dataObject.GetType().GetProperties())
                    dictionary.Add(property.Name, property.GetValue(dataObject));
                ...
                if (!dictionary.ContainsKey("IP"))
                {
                    ...
                    {
                        if (ip_.AddressFamily == AddressFamily.InterNetwork)
                        {
                            ip = ip_.ToString();
                        }
                    }
                ...
                {
                    tbl_UserModel TTNguoiDung = (tbl_UserModel)HttpContext.Current.Session["UserLogIn"];
                    dictionary.Add("UserId", -1);
                    if (TTNguoiDung != null)
                    {
                        dictionary.Remove("UserId");
                        dictionary.Add("UserId", TTNguoiDung.ID);
                    }
                }
