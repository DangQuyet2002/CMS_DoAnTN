...
tableTintuc = () => {
    ...
    table = $.fn.initDataTable("#tableBanner", config);


}

$("#btnThemMoi").on("click", function (e) {
    $.fn.getData('/Admin/AdminBanner/ThemMoi', {}, res => {
        $.fn.showModal({
            title: "THÊM MỚI",
            bodyContent: res,
            dialogClass: "mg"
...
function deleteTable() {
    ...
    if (confirm("Bạn chắc chắn muốn xóa những dữ liệu này?")) {
        $.fn.postData('/Admin/AdminBanner/Delete', { listId: listId.join(",") }, res => {
...
function btnThongTin(Id) {
    $.fn.getData('/Admin/AdminBanner/ChiTiet', { Id: Id }, res => {
        debugger;
        $.fn.showModal({
