...
tableDSAdmin = () => {
    ...
    let config = {
        ...
        fixedHeader: true,
        ajax: {
            url: '/Admin/DMCategory/_DMCategory',
            type: 'POST',
            data: (param) => {
    ...
    table = $.fn.initDataTable("#tableDMCate", config);


}
function btnThongTin(Id) {
    $.fn.getData('/Admin/DMCategory/ChiTiet', { Id: Id }, res => {
        debugger;
        $.fn.showModal({
...
$("#btnThemMoi").on("click", function (e) {
    $.fn.getData('/Admin/DMCategory/ThemMoi', {}, res => {
        $.fn.showModal({
            title: "THÊM MỚI",
            bodyContent: res,
            dialogClass: "mg"
...
deleteTable = () => {
    ...
    if (confirm("Bạn chắc chắn muốn xóa những dữ liệu này?")) {
        $.fn.postData('/DMCategory/Delete', { listId: listId.join(",") }, res => {
